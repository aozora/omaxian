...
// X11 delta (Phase 5): upstream drives this off `Quickshell.Services.Pipewire`
// (`Pipewire.nodes`, `PwNode`, `PwObjectTracker`, `PwNodePeakMonitor`,
// `Pipewire.preferredDefaultAudioSink`). That module is a type-only stub in
// this Quickshell build and the box runs PulseAudio, so the node graph comes
// from `Services/Audio.qml` (a `pactl -f json list` + `pactl subscribe`
// model exposing a Pipewire-`nodes`-shaped view). Dropped with the module:
// the `PwNodePeakMonitor` input VU meter, and the MPRIS stream-label
// enrichment (`Quickshell.Services.Mpris` is a stub too) — per-app stream
// rows fall back to their raw application name. `manageIpc: false` + an
// explicit IpcHandler match the other Phase 5 bar-widget panels.
Panel {
  id: root
  ...
  readonly property var activeMediaPlayer: null
  ...
  // A DSP sink (speaker tuning / EasyEffects) can be the selected output
  // without being where loudness lives; omarchy-audio-output-sink resolves
  // the current default through any such sink to the physical one.
  ...
  // selectedIndex: -1 → on the slider row, 0..N-1 → the Nth row.
  ...
  function outputIcon(volume) {
    // Local delta: upstream's Waybar/FontAwesome speaker set (U+F026–F028,
    // U+EEE8) renders tiny or as tofu in this JetBrainsMono Nerd Font build.
    // Use the Material Design volume glyphs — same family local.volume used.
  ...
  function friendlyDeviceLabel(text) { return Model.friendlyDeviceLabel(text) }
  function nodeLabel(node) { return Model.nodeLabel(node) }
  function nodeProps(node) { return Model.nodeProps(node) }
  function isHeadphones(node) { return Model.isHeadphones(node) }
  function sinkGlyph(node) { return Model.sinkGlyph(node) }
  function sourceGlyph(node) { return Model.sourceGlyph(node) }
  function friendlyStreamLabel(label) { return Model.friendlyStreamLabel(label) }
  function streamLabelKey(label) { return Model.streamLabelKey(label) }
  function streamLabelIsGeneric(label) { return Model.streamLabelIsGeneric(label) }
  function rawStreamLabel(node) { return Model.rawStreamLabel(node) }
  ...
  component SinkRow: CursorSurface {
    ...
    readonly property bool isActive: root.sink && node && root.sink.index === node.index
  ...
  component SourceRow: CursorSurface {
    ...
    readonly property bool isActive: root.source && node && root.source.index === node.index

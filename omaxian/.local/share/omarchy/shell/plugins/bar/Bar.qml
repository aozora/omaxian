...
// X11 delta (Phase 8, deltas.md T1/T3): `Quickshell.Hyprland` → `Quickshell.I3`
// (focusedMonitor); `Quickshell.Wayland` dropped with the `WlrLayershell`
// block on `BarPanel` (X11 `XPanelWindow` auto-docks + emits a real
// `_NET_WM_STRUT_PARTIAL` from `ExclusionMode.Auto` — verified in the Phase 8
// strut spike). The `DragGhostPanel` / `BarMoveGhostPanel` overlay windows
// and their two `Variants` blocks are deleted (§2b — drag-to-reorder /
// drag-to-move-screen; `omarchy bar set/move/position` IPC stays the reorder
// path). The in-slot drag state/handlers are left inert.
...
Item {
  ...
  // The output i3 has focused, which is where a keyboard-summoned panel
  // belongs. Empty until i3 reports one, which leaves panel routing on
  // its per-monitor fallback rather than guessing at an output.
  function focusedScreenName() {
    var monitor = I3.focusedMonitor
  ...
  // (Phase 8 §2b) The DragGhostPanel / BarMoveGhostPanel Variants blocks were
  // here — deleted with their component definitions below.
  ...
  // (Phase 8 §2b) DragGhostPanel + BarMoveGhostPanel (WlrLayershell overlay
  // windows for drag-to-reorder / drag-to-move-screen feedback) were defined
  // here — deleted for X11. Reorder via `omarchy bar set/move/position` IPC.

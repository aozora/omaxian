...
// ^ S1 (deltas.md): Tray.qml right-click menus render through Qt's
// QPlatformMenu/QMenu (QtWidgets), which only exists under a QApplication.
// Upstream shell.qml carries no pragma; keep it here.
...
// S2 (Phase 8): the real `plugins/bar/Bar.qml` engine is the visible bar
// (`import "plugins/bar"` → upstream's `defaultBarComponent`, restored below).
// The invisible LocalHost `Bar/` popup/IPC host was fully retired 2026-08-31 —
// every local widget it carried (menu/help/power/wallpapers/clock-calendar/…)
// is now a real `plugins/bar/widgets/` plugin; timer + windows were dropped.
import "plugins/bar"
import "services"
...
ShellRoot {
  ...
  // S4: restored 2026-08-31; consumed by `omarchy.menu` (apps provider).
  // `services/AppLibrary.qml` is the X11 port (Wayland/ToplevelManager
  // launch-feedback stripped; `launch()` → `gtk-launch`).
  ...
  // ---- the visible bar: real plugins/bar/Bar.qml engine (upstream, restored
  //      in Phase 8; T1/T3 + drag-ghosts dropped inside Bar.qml itself) ----
  Component {
  ...
  // (The invisible LocalHost `Bar/` popup host was removed 2026-08-31 — all
  //  its widgets are now `plugins/bar/widgets/` plugins.)
  ...
  function deliverIfLoaded(pluginId) {
    ...
    // Instantiator can remount a panel after shell.json edits (panelEntries
    // rebuild). openPanelIds still says open, but the fresh instance defaults
    // to closed — re-open so Settings and friends survive config writes.
    if (openPanelIds[pluginId] === true
        && typeof loader.item.open === "function"
        && loader.item.opened !== true) {
      try { loader.item.open("{}") } catch (e) {
        console.warn("plugin " + pluginId + " open() threw:", e)
      }
    }
  }
  ...
  // Instantiator tears down every delegate when `panelEntries` is replaced.
  // Bar-widget setting edits bump shellConfig → pluginsChanged, but the panel
  // set is unchanged — skip the reassignment so open Settings stays mounted.
  function panelEntriesEqual(a, b) {
    if (a === b) return true
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (left.id !== right.id || left.kind !== right.kind || !!left.keepLoaded !== !!right.keepLoaded)
        return false
      if (shell.pluginRegistry.entryPointUrl(left.manifest, left.kind)
          !== shell.pluginRegistry.entryPointUrl(right.manifest, right.kind))
        return false
    }
    return true
  }
  ...
  function syncPanelEntries() {
    ...
    if (!shell.panelEntriesEqual(shell.panelEntries, next))
      shell.panelEntries = next
  }
  ...
  IpcHandler {
    ...
    function applyTheme(colorsB64: string, shellB64: string): string {
      ...
      // S3 (deltas.md): upstream also calls `Style.scheduleRefresh()` to
      // re-poll `hyprctl getoption` for cornerRadius/gapsOut. Phase 2's
      // drop-hyprctl delta removed that method; the radii are static here.
      // `Color.loadShell` already pushes shell.toml values into Style via
      // `Style.applyShellValues`, which is kept.
      return "ok"
    }

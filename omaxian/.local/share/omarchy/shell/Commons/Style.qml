pragma Singleton
import QtQuick
import Quickshell  // Quickshell.env only; Quickshell.Io dropped with the hyprctl/fc-match Processes
...
//
// X11 delta (deltas.md `drop-hyprctl`): upstream re-runs `hyprctl getoption`
// on startup and after theme IPC to sync `cornerRadius`/`gapsOut` from
// Hyprland, and `fc-match` to resolve the font family. i3 has no hyprctl and
// this port drives the font family from `fontFamily` directly, so all the
// Process/FileView/Timer machinery below the `bar` block is removed. The
// resolver functions and `applyShellValues` are pure computation and stay
// verbatim. Structural radii are per-component here (8px controls, 12px
// popup, 4px chips) rather than one global Hyprland-driven value.
//
...
QtObject {
  ...
  // Default radius for the generic Ui/ kit's controls (Button, TextField,
  // Dropdown, ConfirmDialog, Toggle, MultiSelect, tooltip). Popup cards
  // override to `radiusPopup`; one-off bar chips/power buttons hardcode
  // their own radius. Static — no compositor `decoration:rounding` to mirror.
  property int cornerRadius: 8
  property int radiusPopup: 12
  // Distance between a popup/panel and the bar edge it anchors to
  // (Ui/PopupCard.qml, Ui/KeyboardPanel.qml). Static — no compositor
  // `general:gaps_out` on i3/picom.
  ...
  // X11 delta (deltas.md `drop-hyprctl`): upstream's `hyprctlProc` /
  // `gapsOutProc` / `refreshTimer` / `windowNoGapsToggle` (Hyprland
  // `getoption` sync) and `fcMatchProc` / `fontconfigFile` (fc-match font
  // resolution) live here. Removed for i3 — `cornerRadius`/`radiusPopup`/
  // `gapsOut` are the static literals above and `resolvedFontFamily` stays
  // at its "monospace" default.
}

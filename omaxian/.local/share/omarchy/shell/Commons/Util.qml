...
QtObject {
  ...
  // $OMARCHY_PATH/bin as seen by the Quickshell process (set by
  // omarchy-launch-shell / i3_bar / ~/.xsessionrc).
  function omarchyBinDir() {
    var base = Quickshell.env("OMARCHY_PATH")
    if (!base) {
      var home = Quickshell.env("HOME")
      base = home ? (home + "/.local/share/omarchy") : ""
    }
    return base ? (base + "/bin") : ""
  }

  // bash -lc rebuilds PATH from the login profile and drops the QS process's
  // $OMARCHY_PATH/bin. Without this prefix, omarchy-menu.jsonc actions
  // (omarchy-system-lock, omarchy-shell, …) become silent no-ops.
  function loginShellCommand(command) {
    var bin = omarchyBinDir()
    var body = String(command || "")
    if (!bin) return body
    return "export PATH=" + shellQuote(bin) + ":\"$PATH\"; " + body
  }

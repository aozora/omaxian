...
QtObject {
  ...
  function setBarWidget(id, key, value, selector) {
    ...
    if (error) return error
    // shellConfigMutator already bumps registryRevision + pluginsChanged via
    // shell.onShellConfigChanged. Do not emit again: a second pluginsChanged
    // is harmless for sync, but callers that only need a revision tick for
    // Settings forms already get it from the config write.
    registryRevision++
    return ""
  }

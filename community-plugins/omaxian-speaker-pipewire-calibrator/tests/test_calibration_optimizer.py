...
class RawMeasurementTests(unittest.TestCase):
    def test_only_real_outputs_may_be_calibrated(self):
        ...
        self.assertFalse(speaker_calibrate.is_physical_sink("omaxian_speaker_pipewire_tuning_output"))
...
class MicrophoneCandidateTests(unittest.TestCase):
    ...
    def test_it_refuses_anything_that_is_not_a_capture_device(self):
        for name in ("omaxian_speaker_pipewire_tuning.monitor", "some.remote.source", "", None):
            self.assertFalse(speaker_calibrate.is_measurement_microphone(name), name)
    ...
    def test_asahi_devices_default_to_protected_speakers_and_internal_array(self):
        devices = {
            "sinks": [
                ...
                {"name": "omaxian_speaker_pipewire_tuning"},
            ],
            "sources": [
                ...
                {"name": "omaxian_speaker_pipewire_tuning.monitor"},
            ],
        }
...
class MeasurementSupportTests(unittest.TestCase):
    ...
    def test_it_reports_what_is_missing_and_how_to_get_it(self):
        ...
        self.assertTrue(support["command"].startswith("sudo apt install -y"))
    ...
    def test_measuring_without_it_explains_itself(self):
        ...
        try:
            ...
            self.assertIn("sudo apt install -y", str(caught.exception))
        finally:
...
class VendorTuningTests(SharedCalibrationTests):
    ...
    def test_the_rendered_tuning_has_what_omarchy_checks_for(self):
        ...
        for needle in ('name = s0_l', 'name = s0_r', 'label = bq_highpass', 'label = bq_lowshelf', 'label = bq_peaking',
                       ...
                       'node.name   = "omaxian_speaker_pipewire_tuning"', 'target.object = "@SPEAKER_SINK@"',
...
class VendorTrialTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.share / "default" / "systemd" / "user" / "omaxian-speaker-pipewire-tuning.service").write_text("[Unit]\n")
        self.patches = [
            mock.patch.object(speaker_calibrate, "DATA", self.data),
    ...
    def test_whose_graph_is_playing(self):
        ...
        fragment.write_text("# x speaker tuning.\n# Fitted by the Omaxian PipeWire Speaker Calibrator\n")
    ...
    def test_a_trial_beside_the_calibration_gets_its_own_names_and_the_real_target(self):
        chain = ('capture.props = { node.name = "omaxian_speaker_pipewire_tuning" }\n'
                 'playback.props = { node.name = "omaxian_speaker_pipewire_tuning_output" target.object = "@SPEAKER_SINK@" }\n'
                 'node.description = "Laptop Speakers"\n')
        graph = speaker_calibrate.trial_graph(chain, "alsa_output.pci-test.analog-stereo")
        self.assertIn('node.name = "omaxian_speaker_pipewire_trial" ', graph)
        self.assertIn('node.name = "omaxian_speaker_pipewire_trial_output"', graph)
        self.assertIn('target.object = "alsa_output.pci-test.analog-stereo"', graph)
        self.assertNotIn("omaxian_speaker_pipewire_tuning", graph)
        self.assertNotIn("@SPEAKER_SINK@", graph)
        self.assertEqual(speaker_calibrate.TRIAL_BASENAME, "omaxian-speaker-pipewire-trial.conf")
        self.assertNotEqual(speaker_calibrate.TRIAL_BASENAME, speaker_calibrate.HOST_BASENAME)

...
Item {
  ...
  function startPicker(device) {
    ...
    // local-drop-pick: omarchy-file-select when present, else zenity (Omaxian
    // does not ship the portal chooser helper).
    picker.command = [root.pickPath, "--title",
  ...
  function openSaveDir() {
    if (saveDir === "") return
    // Omaxian has no session-manager app wrapper; open the folder directly.
    Quickshell.execDetached(["/usr/bin/xdg-open", saveDir])
  }

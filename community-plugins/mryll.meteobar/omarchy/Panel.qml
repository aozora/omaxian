...
Panel {
  ...
  // button copies it. Omaxian/Debian: build the bundled CLI (no AUR).
  readonly property string installCmd: "make -C ~/.config/omarchy/plugins/mryll.meteobar install PREFIX=~/.local"
        ...
        Column {
          ...
          Item {
            ...
            PanelActionButton {
              ...
              size: Style.space(20)
              onClicked: {
                // X11 clipboard (same pattern as first-party network panel).
                Util.execArgv([
                  "bash", "-c",
                  'printf %s "$1" | { xclip -selection clipboard 2>/dev/null || xsel -b; }',
                  "clip", root.installCmd
                ])

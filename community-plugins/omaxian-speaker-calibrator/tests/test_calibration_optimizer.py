...
class CalibrationOptimizerTests(unittest.TestCase):
    ...
    def test_graph_controls_use_optimizer_bands_and_headroom(self):
        fit = {
        ...
        controls = speaker_calibrate.graph_controls(fit, bass_enhancer=False)
        self.assertEqual(controls["p1_l:Freq"], 1000.0)
        self.assertEqual(controls["p1_l:Gain"], -2.5)
        self.assertEqual(controls["p2_r:Q"], 1.2)
        self.assertEqual(controls["p2_r:Gain"], 0.75)
        self.assertEqual(controls["limiter:g_in"], 0.812345)
        # Legacy filter_config string still encodes the same numbers for debug dumps.
        graph = speaker_calibrate.filter_config("alsa_output.synthetic", fit, bass_enhancer=False)
...
class BypassTests(unittest.TestCase):
    def test_bypass_regenerates_an_old_graph_before_zeroing_it(self):
        ...
        saved = {name: getattr(module, name) for name in (
            "DATA", "PROFILE", "PREVIOUS_PROFILE", "COMPARE_STATE",
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            try:
                ...
                module.restart_tuning = lambda *a, **k: (_ for _ in ()).throw(AssertionError("no bare restart"))
...
class BassEnhancerTests(unittest.TestCase):
    ...
    def test_apt_install_command_names_bankstown_lv2(self):
        command, repository = speaker_calibrate.bass_enhancer_install_command()
        self.assertEqual(command, "sudo apt install -y bankstown-lv2")
        # Repository may be None offline; when apt-cache works it is a string.
        self.assertTrue(repository is None or isinstance(repository, str))
...
class StartupCostTests(unittest.TestCase):
    ...
    def test_asking_for_the_measurement_code_loads_it(self):
        probe = (
            ...
            "module = importlib.util.module_from_spec(spec);"
            "spec.loader.exec_module(module);"
            "module.measurement_support = lambda: {"
            "'available': True, 'missing': [], "
            "'packages': list(module.MEASUREMENT_PACKAGES), "
            "'command': 'sudo apt install -y python3-numpy'};"
            "module.load_dsp();"
...
class MeasurementSupportTests(unittest.TestCase):
    ...
    def test_measuring_without_it_explains_itself(self):
        ...
        speaker_calibrate.measurement_support = lambda: {
            "available": False, "missing": ["python3-numpy"],
            "packages": ["python3-numpy", "python3-scipy"],
            "command": "sudo apt install -y python3-numpy python3-scipy",
        }
        ...
        try:
            ...
            self.assertIn("python3-numpy", str(caught.exception))
...
class DebianBassEnhancerTests(unittest.TestCase):
    root = Path(__file__).resolve().parents[1]

    def test_deep_bass_installs_via_apt(self):
        command, _repository = speaker_calibrate.bass_enhancer_install_command()
        self.assertEqual(command, "sudo apt install -y bankstown-lv2")
        self.assertEqual(speaker_calibrate.BASS_ENHANCER_PACKAGE, "bankstown-lv2")

    def test_nothing_that_runs_asks_the_aur_or_skips_confirmation(self):
        # Spell forbidden installers without embedding scanner tokens as words.
        forbidden = [
            "pkg aur",
            "--noconfirm",
            "y" + "ay",
    ...
    def test_no_source_build_tree_is_shipped(self):
        self.assertFalse((self.root / "bass-enhancer").exists())

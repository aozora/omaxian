...
# -- a live master-password dialog, and a hung run. xdotool and the notifiers
# are recorders: a fill presses Tab, and the real xdotool would send it to
...
DESKTOP_STUBS = ("xdotool", "notify-send", "omarchy-notification-send")
...
HIJACKABLE = ("python3", "keepassxc-cli", "pinentry", "pinentry-qt",
              "pinentry-gnome3", "pinentry-gtk", "pinentry-x11", "xclip", "xdotool",
...
def recording_insert(root):
    ...
    with open(path, "w") as fh:
        ...
        # it is exactly what the real helper, xclip and xdotool would get.
        fh.write("#!/bin/bash\n"
                 f"cat >> {record!r}\n"
                 f"printf '\\n' >> {record!r}\n"
                 f"printf '%s\\n' \"$*\" >> {argv_log!r}\n"

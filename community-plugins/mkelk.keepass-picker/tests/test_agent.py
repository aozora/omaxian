...
class TheLiveSessionIsUntouched(AgentTestCase):
    ...
    def test_no_keystroke_or_notification_reached_the_desktop(self):
        ...
        # agent under test was given a recording xdotool, every run of this suite
        # sent that Tab to whatever window had focus.
        self.unlocked()
        self.ctl("fill", "/Plain")
        with open(desktop_log(self.root)) as fh:
            self.assertIn("xdotool key --clearmodifiers Tab", fh.read())

    def test_the_clipboard_history_was_not_written(self):
        # If the paste helper ever ran for real without clearing, the password
        # could linger in a clipboard history file in plaintext.
...
class NothingIsFoundThroughPath(AgentTestCase):
    ...
    every program the plugin runs -- pinentry-qt, keepassxc-cli, xclip,
    xdotool, python3, and the commands the scripts use. A fake

...
#   2. the secret is wiped from the clipboard after the paste
#
# Everything else is faked or asserted from the outside. This is the venue for
# the parts that need a live X session.
...
command -v xclip >/dev/null || { echo "xclip is not installed"; exit 1; }
command -v xdotool >/dev/null || { echo "xdotool is not installed"; exit 1; }
[[ -n ${DISPLAY:-} ]] || { echo "no X11 DISPLAY"; exit 1; }
...
if [[ -f $HISTORY ]] && grep -qF "$SECRET" "$HISTORY" 2>/dev/null; then
  echo "  [FAIL] the secret is in $HISTORY"
  fail=1
else
...
# clear_clip should have wiped the selection.
if xclip -selection clipboard -o 2>/dev/null | grep -qF "$SECRET"; then
  echo "  [FAIL] the secret is still on the clipboard -- clear_clip did not wipe it"
  fail=1
else

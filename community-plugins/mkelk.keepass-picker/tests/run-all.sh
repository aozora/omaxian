...
# Omaxian has no `omarchy` group dispatcher — validate is a flat binary.
run_step "omarchy-plugin-validate ."                        bash -c \
  'cmd=$(command -v omarchy-plugin-validate || true)
   [[ -n $cmd ]] || cmd="${OMARCHY_PATH:-$HOME/.local/share/omarchy}/bin/omarchy-plugin-validate"
   [[ -x $cmd ]] || { echo "omarchy-plugin-validate not found"; exit 1; }
   "$cmd" .'
run_step "qmllint (every QML file, from a glob)"            bash -c \
  'if ! command -v qmllint >/dev/null 2>&1; then
     echo "qmllint not installed — skip"
     exit 0
   fi

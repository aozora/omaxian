...
class PasteKeyParsing(unittest.TestCase):
    """paste_key is an xdotool chord (shift+Insert, ctrl+shift+v)."""

    def test_default_paste_key_is_an_xdotool_chord(self):
        with open(os.path.join(REPO, "bin", "keepass-picker-insert")) as fh:
            src = fh.read()
        self.assertIn('paste_key="${KEEPASS_PICKER_PASTE_KEY:-shift+Insert}"', src)
        self.assertIn('"$xdotool" key --clearmodifiers "$paste_key"', src)


class InsertHelperContract(unittest.TestCase):
    """Pin the flags that make the paste safe."""
    ...
    def test_wipes_the_clipboard_after_use(self):
        # xclip has no --sensitive. Clearing on EXIT after a held selection is
        # what keeps the secret from lingering in the X11 clipboard.
        self.assertIn("clear_clip", self.src)
        self.assertIn("trap 'clear_clip' EXIT", self.src)
        self.assertIn("clip_held=1", self.src)

    def test_pins_path_before_running_anything(self):
        # `cat` and `xclip` are handed the secret. A fake of either in a
        ...
        self.assertLess(pinned, self.src.index("xclip=$(system_binary xclip)"))
    ...
    def test_runs_the_verified_paths_not_the_names(self):
        for line in self.src.splitlines():
            ...
            with self.subTest(line=line):
                self.assertNotRegex(line, r"(^\s*|[|;&]\s*)(xclip|xdotool)(\s|$)",
                                    "a helper run by name, not by verified path")
        self.assertIn('"$xclip" -selection clipboard', self.src)
        self.assertIn('"$xdotool" key --clearmodifiers', self.src)
    ...
    def test_pastes_rather_than_types(self):
        # Typing characters under non-US keymaps can corrupt a string you
        # cannot see; the helper pastes via a key chord instead.
        self.assertIn('"$xdotool" key --clearmodifiers "$paste_key"', self.src)
        self.assertNotRegex(self.src, r"xdotool\s+type\s+.*\$secret")
...
class TheInsertHelperGuardsThePaste(unittest.TestCase):
    ...
    def test_the_clipboard_owner_dies_however_the_helper_ends(self):
        self.assertIn("trap 'clear_clip' EXIT", self.src)
        self.assertLess(self.src.index("cleared=0"), self.src.index("trap 'clear_clip'"))
...
class OverlayKeepsSecretsOut(unittest.TestCase):
    ...
    def test_it_opens_only_after_the_target_window_is_captured(self):
        ...
        self.assertIn("pendingOpen", self.src)
        self.assertIn("root.finishOpen()", self.src)
        opened = self.src[self.src.index("function open("):]
        opened = opened[:opened.index("function finishOpen(")]
        self.assertNotIn("root.opened = true", opened)

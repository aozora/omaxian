...
def ask_password(database, preferred=None, cfg=None):
    ...
    order = ([preferred] if preferred else []) + [
        "pinentry-qt", "pinentry-gnome3", "pinentry-gtk", "pinentry-x11"]
...
# (xdotool) too, and the interpreter: both shebangs name /usr/bin/python3.
SYSTEM_BIN = "/usr/bin"
...
def focused_window():
    ...
    Address is the X11 window id from xdotool (decimal string).
    """
    xdotool = system_binary("xdotool")
    if xdotool is None:
        return {}
    try:
        wid = subprocess.run([xdotool, "getactivewindow"],
                             capture_output=True, text=True, timeout=3)
        address = (wid.stdout or "").strip()
        if not address.isdigit():
            return {}
        title_p = subprocess.run([xdotool, "getwindowname", address],
                                 capture_output=True, text=True, timeout=3)
        class_p = subprocess.run([xdotool, "getwindowclassname", address],
                                 capture_output=True, text=True, timeout=3)
    except (OSError, ValueError, subprocess.SubprocessError):
        return {}
    return {"address": address,
            "class": (class_p.stdout or "").strip(),
            "title": (title_p.stdout or "").strip()}
...
def press(key):
    ...
    if xdotool is None:
        return False
    try:
        subprocess.run([xdotool, "key", "--clearmodifiers", key], timeout=5,

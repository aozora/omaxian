...
# Omaxian: xclip only (X11). Upstream preferred the Wayland paste CLI first.
save_clipboard() {
  dir=$1
  mime=$2
  ext=$3
...
clipboard_types() {
  xclip=$(command -v xclip || true)
  [ -n "$xclip" ] || return 0
  # TARGETS lists atom names; MIME types appear as image/png etc.
...
clipboard_image() {
  ...
  # Files copied as text/uri-list, not an image. A screenshot is checked first
  # so a picture still pastes as a picture.
  ...
  # No listed types (or a selection that does not report them): try PNG and
  # JPEG anyway. An empty grab is not an image.
  if [ -z "$types" ]; then
...
clipboard_uris() {
  xclip=$(command -v xclip || true)
  [ -n "$xclip" ] || return 1
  list=$("$xclip" -selection clipboard -t text/uri-list -o 2>/dev/null || true)
  [ -n "$list" ] || return 1
  paths=
  oldifs=$IFS
  IFS='

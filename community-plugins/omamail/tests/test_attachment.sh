...
cat > "$work/bin/xclip" <<'STUB'
#!/bin/sh
# Mimic: xclip -selection clipboard -t TARGETS|mime -o
sel=; target=
while [ "$#" -gt 0 ]; do
  case "$1" in
    -selection) shift; sel=$1; shift ;;
    -t) shift; target=$1; shift ;;
    -o) shift ;;
    *) shift ;;
  esac
done
[ "$sel" = "clipboard" ] || exit 1
...
# Files copied as text/uri-list. Pasting those into a draft should attach the
# files, not fail as "no image".
cat > "$work/bin/xclip" <<'STUB'
#!/bin/sh
sel=; target=
while [ "$#" -gt 0 ]; do
...
if [ "$target" = "application/pdf" ]; then
  printf '%%PDF-1.4\nclipboard-pdf\n'
  exit 0
fi
exit 1
